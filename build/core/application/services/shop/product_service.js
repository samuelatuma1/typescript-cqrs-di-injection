...
const update_product_inventory_type_1 = require("../../../domain/shop/enum/update_product_inventory_type");
let ProductService = class ProductService {
    ...
    buildUpdateDataForNonListFields = (product, updateProductRequest) => {
        const updateData = {};
    ...
    buildAddAndRemoveFieldListsForProduct = async (updateProductRequest) => {
        let addToFieldsUpdate = {};
        let removeFromFieldsUpdate = {};
        ...
        if (updateProductRequest.addCategories) { // save only existing categories
            let categoriesIds = updateProductRequest.addCategories.map(categoryId => new mongoose_1.Types.ObjectId(categoryId));
        ...
        if (updateProductRequest.removeCategories) {
            removeFromFieldsUpdate.categories = updateProductRequest.removeCategories.map(categoryId => new mongoose_1.Types.ObjectId(categoryId));
        }
    ...
    updateProduct = async (productId, updateProductRequest) => {
        ...
        try {
            ...
            if (updateProductRequest.filters && object_utility_1.default.objectSize(updateProductRequest.filters)) {
                let categoryIds = [...product.categories, ...(addToFieldsUpdate?.categories ?? [])];
